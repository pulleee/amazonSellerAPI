...
TWawi::TWawi(TADOConnection *con)
{
	...
	// User ermitteln
	s="select kuerzel from SL_Daten.dbo.[PASSWORT] where sqlserverbenutzer = (select login_name collate Latin1_General_CS_AS from sys.dm_exec_sessions where session_id = @@SPID)";
	q->SQL->Text=s;
	q->Open();
	if(q->RecordCount)
...
double TWawi::getArtikelVKPreis(UnicodeString sArtikelnummer, UnicodeString sWaehrung, double dMenge, int iPreisgruppe, UnicodeString sKundennummer)
{
	...
	s="select top 1 * from arpreis where artikelnummer='"+sArtikelnummer+"' and preistyp='K' and typnummer='"+sKundennummer+"' and menge <= '"+getSQLFloat(dMenge)+"' order by menge desc";
	q->SQL->Text=s;
	q->Open();
	if(q->RecordCount)
...
bool TWawi::addBestand(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, double dWert, AnsiString sSerieCharge)
{
	AnsiString s;
	...
	s+=
		...
		"insert into lagerprotokoll (BELEGPKennung,	Typ, Tabelle,	Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
		 ...
		 "Values('"+sBelegpKennung+"', @typ, '"+IntToStr(ltaLagerbestand)+"', CONVERT(date, GETDATE()), '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', '0', @menge, @wert, "
		...
		"BEGIN "
			...
			//"else begin "
				...
				"Values(@Id, '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', @zugang, @wert, '"+sSerieId+"', '1', @verfallsdatum, @lagerplatzid, NULL, '"+this->_sUser+"', GetDate()) "
			//"end "
			//Protokoll
			"insert into lagerprotokoll (BELEGPKennung,	Typ, Tabelle,	Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
																	"SerieId, SCTyp, SerieCharge,	Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm) "
			"Values('"+sBelegpKennung+"', @typ, '"+IntToStr(ltaLagerbestand)+"', CONVERT(date, GETDATE()), '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', '0', @zugang, @wert, "
...
bool TWawi::subBestand(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, AnsiString sSerieCharge)
{
	AnsiString s;
	...
	s+=
		...
		"  insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
																	"SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm) "
		"  Values('"+sBelegpKennung+"', '"+IntToStr(ltyAbgang)+"', '"+IntToStr(ltaLagerbestand)+"', CONVERT(date, GETDATE()), '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', '0', @menge, @wert,"
						"'"+sSerieId+"', '"+sSCTyp+"', '"+sSerieCharge+"', 1.0, @verfallsdatum, '"+this->_sStandort+"', '"+sLager+"', @lagerplatzid, NULL, @TabId, '"+this->_sUser+"', GetDate()) "
		" "
		"  if(@abgang <= 0) "
		"    break "
		...
		"BEGIN "
			...
			//"else begin "
				"insert into lagerbestand (ID, Datum, Artikelnummer, Bestand, Wert, SerieId, Preismenge, Verfallsdatum, LagerplatzId, BelegpKennung, AngelegtVon, AngelegtAm) "
				"Values(@Id, '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', @abgang*-1, @wert, '"+sSerieId+"', '1', @verfallsdatum, @lagerplatzid, NULL, '"+this->_sUser+"', GetDate()) "
			//"end "
			//Protokolleintrag einfügen
			"insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
																	"SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm) "
			"Values('"+sBelegpKennung+"', '"+IntToStr(ltyAbgang)+"', '"+IntToStr(ltaLagerbestand)+"', CONVERT(date, GETDATE()), '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', '0', @abgang*-1, @Wert,"
						 "'"+sSerieId+"', '"+sSCTyp+"','"+sSerieCharge+"', 1.0, @verfallsdatum, '"+this->_sStandort+"', '"+sLager+"', @lagerplatzid, NULL, @Id, '"+this->_sUser+"', GetDate()) "
		"END "
		"END TRY "
		"BEGIN CATCH "
...
bool TWawi::setReserviert(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, double dWert)
{
	...
	s=
		...
		"else begin "
			...
			"Values(@Id, 1, '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', @menge, '"+sLager+"', @lagerplatzid, '"+this->_sStandort+"','"+this->_sUser+"', GetDate()) "
		"end "
		//Protokoll
		"insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
																"SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm) "
		"Values('"+sBelegpKennung+"', @typ, '"+IntToStr(ltaLagerbedarf)+"', CONVERT(date, GETDATE()), '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', @menge, 0, '"+sWert+"',"
...
bool TWawi::setBestellt(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, double dWert)
{
	...
	s=
		...
		"declare @lagerbedarf_id int "
		"set @lagerbedarf_id=(select lagerbedarf_id from lagerbedarf where id=(select top 1 tabid from lagerprotokoll where belegpkennung='"+sBelegpKennung+"')) "
		"declare @wert float "
		"set @wert='"+sWert+"' "
		"declare @menge float "
		"set @menge='"+sMenge+"' "
		...
		"else begin "
			"insert into lagerbedarf (ID, Typ, Datum, Artikelnummer, Menge, Lager, LagerplatzId, Standort, AngelegtVon, AngelegtAm) "
			"Values(@Id, 2, '"+tdDatum.DateString()+"', '"+sArtikelnummer+"', @menge, '"+sLager+"', @lagerplatzid, '"+this->_sStandort+"','"+this->_sUser+"', GetDate()) "
		"end "
		//Protokoll
		"insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert, "
																"SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm) "
...
bool TBeleg::edit(UnicodeString sBelegtyp, UnicodeString sBelegnummer)
{
	...
	if(q->RecordCount)
		...
		this->sLiefertermin=q->FieldByName("Liefertermin")->AsString;
...
bool TBeleg::save(void)
{
	...
	if(_bLieferAdressChanged)
		...
		s="update adress set "
			...
			"if @@rowcount=0 "
				"insert into adress (Adresstyp, Name, Vorname, Firma, Zusatz, Zusatz2, Zusatz3, Strasse, PLZ, Ort, Land, ILNNummer) "
...
bool TBelegp::edit(UnicodeString sKennung)
{
	...
	if(q->RecordCount)
		...
		//this->sBelegnummer=sBelegnummer;
		//this->sBelegtyp=sBelegtyp;
		this->tdDatum=q->FieldByName("Datum")->AsString;
...
bool TBelegp::save(void)
{
	...
	if(sKennung == "")  // die Position noch nicht existiert
		...
		s="DECLARE @myid uniqueidentifier "
			...
			"Values('"+_beleg->getBelegtyp()+"', '"+_beleg->getBelegnummer()+"', '"+_beleg->getStandort()+"', '"+sZeilentyp+"', @myid, NULL, GetDate(), '"+_beleg->getUser()+"') "
			" "
			"select @myid kennung";
		q->SQL->Text=s;
		q->Open();
	...
	if(_beleg->getLagerFlag() == "A" && this->bLagerartikel)     // auslagern
		...
		s="select sum(bestand) as bestand from lagerprotokoll where typ='3' and belegpkennung='"+sKennung+"' and artikelnummer='"+sArtikelnummer+"'";
		q2->SQL->Text=s;
		q2->Open();
		if(q2->RecordCount)
		...
		if(sVorgaengerpKennung != "")
			...
			if(q2->RecordCount)
				...
				if(sTyp == 1) // wenn eine Reservierung existiert
					...
					if(dMenge < dVReserviert)                            // wenn nicht die komplette Reserierung aufgelöst wird
						dVReserviert=dMenge;                               // begrenzen auf die Menge
					...
					dNochZuLagern=dMenge-dVGelagert;
					}
		...
		{
		// bereits gelagerte Mengen dieser Position checken
		s="select sum(bestand) as bestand from lagerprotokoll where typ='4' and belegpkennung='"+sKennung+"' and artikelnummer='"+sArtikelnummer+"'";
		q2->SQL->Text=s;
		q2->Open();
		if(q2->RecordCount)
		...
		if(sVorgaengerpKennung != "")
			...
			if(q2->RecordCount)
				...
				if(sTyp == 2) // wenn eine Bestellung existiert
					...
					if(dMenge < dVBestellt)                              // wenn nicht die komplette Bestellung aufgelöst wird
						dVBestellt=dMenge;                                 // begrenzen auf die Menge
		...
		if(sVorgaengerpKennung != "")
			...
			if(q2->RecordCount)  // wenn ja, dann bestimmt diese Menge auch die Ueberabe/Verwendet Mengen
				...
				if(sTyp == 1)      // wenn eine Reservierung existiert
					{
					dVReserviert=q2->FieldByName("menge")->AsFloat*-1;    // Menge kommt im - -> gerechnet wird positiv
					dNochZuReservieren=dMenge-dVReserviert;
					}
				else if(sTyp == 3)
					{
					dNochZuReservieren=dMenge;
					}
				else
					dNochZuReservieren=dMenge;
				}
		...
		if(sVorgaengerpKennung != "")
			...
			if(q2->RecordCount)
				...
				if(sTyp == 2) // wenn eine Bestellung existiert
					{
					dVBestellt=q2->FieldByName("menge")->AsFloat;
					dNochZuBestellen=dMenge-dVBestellt;
					}
				else if(sTyp == 4)  // Lagerbestand
					{
					dNochZuBestellen=dMenge;
					}
				else
					dNochZuBestellen=dMenge;
				}
	...
	if(sStueckliste == "H")
		...
		double dMenge;                        /* TODO : Preisverteilung auf Stücklistenartikel integrieren */
		double dHPosEP, dGPosEP;
		dHPosEP=this->getEinzelpreis();
		s="select * from artset where artikelnummer='"+sArtikelnummer+"'";
		q->SQL->Text=s;
		q->Open();
		for(int i=0; i < q->RecordCount; i++)
			{
	...
	if(bSt)
      ...
      q2->Close();
		...
		if(_beleg->getAdressBlobKey() == "KU")
			...
			this->dEP=_beleg->getArtikelVKPreis(this->sArtikelnummer, _beleg->getWaehrungscode(), this->dMenge, _beleg->getPreisgruppe(), _beleg->getAdresse());
			...
			// Einzelpreis Preis holen
			this->dEP=_beleg->getArtikelEKPreis(this->sArtikelnummer, this->dMenge, _beleg->getAdresse());
			//if(_beleg->getWaehrungscode() != "EUR")
			//	this->dEP/=_beleg->getWaehrungsfaktor(_beleg->getWaehrungscode(), _beleg->getDatum());
		...
		q->Open();
		if(q->RecordCount)
			this->dKalk=q->FieldByName("Kalkulationspreis")->AsFloat;
		else
			this->dKalk=0;
		q->Close();
...
void TWerkauftrag::setLagerEin(UnicodeString sLager)
{
	...
	s="update belegp set lager='"+sLager+"' where belegtyp='W' and belegnummer='"+this->getBelegnummer()+"' and postext=''";
	q->SQL->Text=s;
	q->ExecSQL();
...
bool TWerkauftrag::setUnReserviertPos(UnicodeString sKennung, double dMenge)
{
	...
	*/
	...
	s="BEGIN TRANSACTION; "
		...
		"DECLARE cur CURSOR SCROLL FOR  "
		"select BELEGPKennung, Datum, Artikelnummer, Menge, Standort, Lager, LagerplatzId, TabID from lagerprotokoll  "
		"  where belegpkennung in (select lagerkennung from WERKAUFTRAGDETAILS where typ='1' and belegpkennung='"+sKennung+"') "
		"OPEN cur "
		"FETCH NEXT FROM cur INTO @belegpkennung, @datum, @artikel, @menge, @standort, @lager, @lagerplatzid, @tabid  "
		"WHILE @@FETCH_STATUS = 0  "
		"BEGIN  "
...
bool TWerkauftrag::setEingelagert(double dMenge)
{
	...
	*/
	...
	s="BEGIN TRANSACTION; "
		...
		" "
		"DECLARE cur CURSOR SCROLL FOR  "
		"select lp.BELEGPKennung, lp.Datum, lp.Artikelnummer, lp.Menge, lp.Standort, bp.Lager, lp.Lager, lp.LagerplatzId, lp.TabID, bp.Kennung from lagerprotokoll lp  "
		"  left join WERKAUFTRAGDETAILS wd on (wd.LagerKennung=lp.BELEGPKennung) "
		"  left join belegp bp on (bp.kennung=wd.BELEGPKennung) "
		"where bp.belegtyp='W' and bp.belegnummer=@belegnummer and bp.postext='' and wd.typ='6'  "
		" "
		"OPEN cur "
		"FETCH NEXT FROM cur INTO @belegpkennung, @datum, @artikel, @menge, @standort, @lager, @lagerplatzid, @tabid, @kennung "
		"WHILE @@FETCH_STATUS = 0  "
		...
		"insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert,  "
		"  SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm)  "
		"Values(@belegpkennung, '6', '4', CONVERT(date, GETDATE()), @datum, @artikel, -@menge, 0, '0', "
		"  '0', '', '', 1.0, NULL, @standort, @lager, @lagerplatzid, NULL, @tabid, @user, GetDate()) "
		" "
		...
		"  insert into lagerprotokoll (BELEGPKennung, Typ, Tabelle, Datum, Bestandsdatum, Artikelnummer, Menge, Bestand, Wert,  "
		"    SerieId, SCTyp, SerieCharge, Preismenge, Verfallsdatum, Standort, Lager, LagerplatzId, TabBELEGPKennung, TabID, AngelegtVon, AngelegtAm)  "
		"  Values(@belegpkennung, '4', '2', CONVERT(date, GETDATE()), @datum, @artikel, '0', @menge, @wert, "
		"@serieId, @scTyp, @serieCharge, 1.0, NULL, @standort, @lager, @lagerplatzid, NULL, @TabId, @user, GetDate())  "
		"  "
		"  if(@zugang <= 0)  "
		"    break  "
