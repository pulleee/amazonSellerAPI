...
class TWawi
{
	...
	UnicodeString getErloeskontoFromMatrix(UnicodeString sKundenKontengruppe, UnicodeString sArtikelkontengruppe, UnicodeString &sAbwieichenderSteuercode, int &iOSSVerfahren);
  ...
  double getBestand(UnicodeString sArtikelnummer, UnicodeString sLager);
	bool addBestand(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, double dWert=0, AnsiString sSerieCharge="");
	bool subBestand(UnicodeString sBelegpKennung, TDateTime tdDatum, UnicodeString sArtikelnummer, UnicodeString sLager, double dMenge, AnsiString sSerieCharge="");
	...
	AnsiString _strVersion1, _strVersion2, _strVersion3;
...
class TBeleg : public TWawi
{
  ...
  TADOConnection *getCon(void) {return(_con);}
	...
	void setLiefertermin(TDateTime dtLiefertermin) {this->sLiefertermin=dtLiefertermin.DateString(); _bHeaderChanged=true;}
...
class TBelegp
{
  ...
  int iOSSVerfahren;
	...
	void setMengeneinheit(UnicodeString sME) {this->sME=sME;}
	void setEinzelpreis(double dEinzelpreis) {dEP=dEinzelpreis; calc();}
	void setRabatt(double dRabatt) {this->dRabatt=dRabatt; calc();}
	void setPreiseinheit(double dPreiseinheit) {dPE=dPreiseinheit; calc();}
	void setTermin(TDateTime tdTermin) {this->sTermin=tdTermin.DateString();}
	...
	void setLager(UnicodeString sLager) {this->sLager=sLager;}
	void setKalkpreis(double dKalk) {this->dKalk=dKalk; calc();}
