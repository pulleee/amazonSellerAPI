...
UnicodeString TWawi::getErloeskontoFromMatrix(UnicodeString sKundenKontengruppe, UnicodeString sArtikelkontengruppe, UnicodeString &sAbweichenderSteuercode)
{
      ...
      q2->Close();
		...
		if(_beleg->getAdressBlobKey() == "KU")
			...
			if(q->FieldByName("KontoVerkauf")->AsString != "")
				...
				this->sKonto=_beleg->getErloeskontoFromMatrix(q->FieldByName("Erfolgskontengruppe")->AsString, _beleg->getKontengruppe(), sAbwSteuercode);
				if(sAbwSteuercode != "")
